...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		$this->file = WP_PARTNERSHIPM__FILE__;
		$this->plugin_slug = 'wp-partnershipm';
		$this->plugin_name = 'wp-partnershipm';
		$this->textdomain = 'wp-partnershipm'; // str_replace('_', '-', $plugin_slug);
		// Initialise settings
	...
	public function init() {
		$this->general = (object) wp_parse_args(apply_filters('pm_project/settings/general', []), [
			'page_title'					=> __('Indepth Settings', 'wp-partnershipm'),
			'menu_title'					=> __('Settings', 'wp-partnershipm'),
			'role'							=> 'manage_options',
			'slug'							=> $this->plugin_slug,
			'page_header'					=> __('Application in depth configuration screen.', 'wp-partnershipm'),
			'page_subheader'				=> __('....', 'wp-partnershipm'),
			'no_password'					=> __('A password is required.', 'wp-partnershipm'),
		]);
	}
	...
	public function add_settings_link($links) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __('Settings', 'wp-partnershipm') . '</a>';
		// array_push($links, $settings_link);
		$links[] = $settings_link;
		$links[] = '<a href="https://www.fiverr.com/mahmud_remal/" target="_blank">' . __('Developer Support', 'wp-partnershipm') . '</a>';
		return $links;
	}
	/**
	...
	public function getOption($option, $default) {
		return isset(WP_PARTNERSHIPM_OPTIONS[$option])?WP_PARTNERSHIPM_OPTIONS[$option]:$default;
	}
	/**
	...
	public function isActive($option) {
		return (isset(WP_PARTNERSHIPM_OPTIONS[$option]) && WP_PARTNERSHIPM_OPTIONS[$option] == 'on');
	}
}
          ...
          {(invoiceData && showOverview) && (
            <div className="xpo_w-full xpo_h-full xpo_bg-gray-50 xpo_px-8 xpo_py-4 xpo_border-l xpo_flex xpo_flex-col xpo_justify-between xpo_sticky xpo_top-0">
              ...
              <div>
                ...
                {!invoiceError && <SharePopup url={location.href} title={__('Share payment link')} __={__} />}
              </div>
            </div>
          )}
        </form>
      ...
      {open && (
        <div className="xpo_absolute xpo_top-full xpo_right-0 xpo_mt-2 xpo_bg-white xpo_border xpo_rounded-lg xpo_shadow-lg xpo_p-4 xpo_z-50 xpo_w-64" ref={popupRef}>
          <div className="xpo_flex xpo_flex-wrap xpo_gap-4">
            ...
            <button onClick={() => window.open(`https://twitter.com/messages/compose?recipient_id=USER_ID&text=${encodeURIComponent(`${title}: ${url}`)}`, '_blank')} className="xpo_w-10 xpo_h-10">
              <svg width={30} height={30} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                <path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z" />
              </svg>
            </button>
            <button onClick={() => window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(title)}%20${encodeURIComponent(url)}`, '_blank')} className="xpo_w-10 xpo_h-10">
            ...
            <button onClick={() => window.open(`mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(url)}`)} className="xpo_w-10 xpo_h-10">
              <svg width={30} height={30} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">

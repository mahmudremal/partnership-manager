...
export default function Checkout() {
    ...
    const fetchThings = async () => {
        setLoading(true);
        try {
            request(rest_url(`/partnership/v1/contracts/packages/${package_id}`)).then(packs => setPricing(packs));
            request(rest_url(`/partnership/v1/payment/gateways`)).then(data => Object.keys(data).map(k => ({id: k, ...data[k]}))).then(data => {
                setGateways(data);
                ...
                <div className="card h-100 p-0 radius-12">
                    <div className="card-body px-24 py-32">
                        ...
                        <div className="border radius-12 p-24">
                            <h6 className="text-md mb-16">{pricing?.list_title??'What’s included'}</h6>

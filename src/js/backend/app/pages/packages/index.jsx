...
export default function Packages({ viewType = 'list' }) {
    ...
    useEffect(() => {
        request(rest_url(`/partnership/v1/stores?per_page=50`), {headers: {'Content-Type': 'application/json'}})
                ...
                <div>
                    <ul className="nav nav-pills button-tab pricing-tab justify-content-center xpo_m-0">
                        ...
                        <li className="nav-item" key={index}>
                            <button onClick={() => setPlan(p)} className={ `nav-link text-md rounded-pill text-secondary-light fw-medium ${plan == p && 'active'}` }>{__(p)}</button>
                        </li>
                        )}
                    </ul>
                </div>
...
const InvoiceGenerator = ({ pack, plan, stores, __ }) => {
    ...
    const handle_package_submit = async (e, pack, plan) => {
        ...
        request(rest_url(`/partnership/v1/contracts/packages/${pack.id}/${plan}/create`), {headers: {}, body: fdata, method: 'POST'})
        .then(res => {
            if (res?.invoice_id) {
                // setPopup(null);

...
export default function Stores() {
    ...
    const fetchStores = async () => {
        ...
        request(rest_url(`/partnership/v1/stores?${queryString}`), {headers: {'Content-Type': 'application/json'}})
        .then(res => {
            setStores(res?.list??[]);
    ...
    const handle_delete = (e, store) => {
        ...
        if (store) {
            setLoading(true);
            request(rest_url(`/partnership/v1/store/${store.id}`), {method: 'DELETE'})
                                                                    ...
                                                                    {store?.plans?.length && (
                                                                        <div>
                                                                            ...
                                                                            <div className="xpo_w-full xpo_flex xpo_gap-2">
                                                                                {(store?.plans??[]).map(p => <span className="badge text-sm fw-semibold rounded-pill text-success-600 bg-success-100 px-20 py-9 radius-4 text-white">{p}</span>)}
                                                                            </div>
                                                                        </div>
                                                                    )}
                                                                </p>
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => setPopup(
                                                            <div className="xpo_relative xpo_max-w-sm xpo_flex xpo_flex-col xpo_gap-5">
                                                                <h6 class="xpo_text-primary-500 text-lg fw-semibold">{__('Are you sure you want to delete this store?')}</h6>
...
const EditStore = ({ data = {}, variants = [], __, setPopup, onSuccess }) => {
    ...
    const handle_submit = async (event) => {
        ...
        setLoading(true);
        await sleep(2000);
        request(rest_url(`/partnership/v1/store/create`), {
            method: 'POST',
        ...
        <div className="xpo_max-w-md">
            <form method="post" autoComplete="off" encType="multipart/form-data" onSubmit={(e) => handle_submit(e)}>
                <div className="row gy-3">
                    ...
                    <button type="submit" className={ `btn rounded-pill radius-8 px-20 py-11 xpo_flex xpo_items-center xpo_gap-3 xpo_justify-center ${loading ? 'btn-secondary-light text-secondary-light' : 'btn-success-100 text-success-600'}` } disabled={loading}>
